...
╚═╝        ╚══▀▀═╝ ╚═════╝     ╚═╝    ╚═════╝ ╚═════╝ ╚══════╝     ''')
    thanhngang(65)
    print(f'''{thanh} {luc}Admin{trang}: {vang}PHẠM QUANG DŨNG 
{thanh} {luc}Zalo{trang}: {red}0336502026
{thanh} {luc}MBBANK{trang}: {red}0336502026
{thanh} {luc}TELEGRAM{trang}: {red}@CHUOTHAMTO          
...
def Delay(value):
	while not(value <= 1):
		value -= 0.123
		print(f'''{trang}[{lam}PQDTOOL{trang}] [{lam}DELAY{trang}] [{lam}{str(value)[0:5]}{trang}] [{vang}X    {trang}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{trang}[{lam}PQDTOOL{trang}] [{lam}DELAY{trang}] [{lam}{str(value)[0:5]}{trang}] [ {vang}X   {trang}]''', '               ', end = '\r')
		sleep(0.02)
		print(f'''{trang}[{lam}PQDTOOL{trang}] [{lam}DELAY{trang}] [{lam}{str(value)[0:5]}{trang}] [  {vang}X  {trang}]''', '               ', end = '\r')
